    ...
    for (const filter of filters) {
      ...
      if (filter.domain) {
        if (Array.isArray(filter.domain)) {
          if (!filter.domain.some(domain => hostname.includes(domain))) {
      ...
      if (conditionMatches && filter.pathPrefix) {
        if (Array.isArray(filter.pathPrefix)) {
          if (!filter.pathPrefix.some(prefix => pathname.startsWith(prefix))) {
            conditionMatches = false;
          }
      ...
      if (conditionMatches && filter.regex) {
        if (Array.isArray(filter.regex)) {
          if (!filter.regex.some(pattern => new RegExp(pattern).test(url))) {
            conditionMatches = false;
          }
      ...
      if (conditionMatches && filter.keywords) {
        if (Array.isArray(filter.keywords)) {
          if (!filter.keywords.some(keyword => url.includes(keyword))) {
            conditionMatches = false;
          }
